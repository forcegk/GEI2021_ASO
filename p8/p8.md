...
main() {
	...
	# Creamos el archivador *somehow*
	du -ks $(getent passwd | cut -d":" -f6 | awk $AWK_EXCLUDE_REGEX) | \
	awk '$1 >= blim {print $2}' blim="$BACKUP_LIM" | \
	xargs -I {} sh -c 'tar -cf - -C {}/.. $(basename {}) | gzip -c9 > /var/backups/$(basename {}).tar.gz && chown $(basename {}):root /var/backups/$(basename {}).tar.gz && chmod 400 /var/backups/$(basename {}).tar.gz'
