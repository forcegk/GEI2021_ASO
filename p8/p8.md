...
main() {
	...
	DIR_BACKUP_FILTERED=`du -ks $(getent passwd | cut -d":" -f6 | awk $AWK_EXCLUDE_REGEX) | awk '$1 >= blim {print >FILENAME}' blim="$BACKUP_LIM" | cut -f2`
	
	 | xargs -I {} sh -c 'tar -cf - {} | gzip -c9 > /var/backups/$(echo {} | rev | cut -d"/" -f1 | rev).tar.gz'
