...
0       3       *       *       *       -s /home/pc/backup.sh && newsyslog -Ff /etc/newsyslog-rotate-backups.conf
```
...
0       3       *       *       *       -s find /var/backups/ -name "*.tar.gz" -exec echo "{} 400 7 * @T03 B" \; | column -t | newsyslog -Ff - && /home/pc/backup.sh
```
