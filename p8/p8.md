...
main() {
    ...
    du -ks `getent passwd | cut -d":" -f1,6 | $awk $AWK_EXCLUDE_REGEX` | \
    $awk 'function basename(dir) { sub(".*/", "", dir); return dir;} $1 >= blim {print $2, basename($2)}' blim="$BACKUP_LIM" | \
    xargs -L1 sh -c "tar -cf - -C \$0/.. \$1 | gzip -c$BACKUP_COMPRESSION_LEVEL > $BACKUP_DIR/\$1.tar.gz && chown \$1 $BACKUP_DIR/\$1.tar.gz && chmod 400 $BACKUP_DIR/\$1.tar.gz && echo \"Backed up user \$1 at $BACKUP_DIR/\$1.tar.gz\""


	## cambios staging
	UN_PAS=`getent passwd | cut -d":" -f1,6 | $awk $AWK_EXCLUDE_REGEX`
	DIRSIZE=`du -ks $(echo $UN_PAS | cut -d":" -f2)`
